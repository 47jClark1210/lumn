...
async function getAllModules() {
  const result = await db.query(
    'SELECT * FROM learning_modules ORDER BY created_by DESC, id DESC',
  );
  return result.rows;
}
...
async function updateModule(
  id,
  { title, description, resource_url, type, tags },
) {
  const result = await db.query(
...
async function deleteModule(id) {
  const result = await db.query('DELETE FROM learning_modules WHERE id = $1', [
    id,
  ]);
